...
Fsw.prototype.connect = function() {
  ...
  async.series([
	  ...
	  var re = new RegExp('^\\s*' + self.profile + '\\s.*sip:[^"]+@((?:[0-9]{1,3}\\.){3}[0-9]{1,3}):(\\d+)', 'm') ;
          ...
          logger.info('connected to freeswitch at %s:%s, it reports handling SIP on %s:%d', self.address, self.profile, 
            self.sipAddress, self.sipPort) ;
          callback(null) ;
        }) ;
      }
    ], 
...
Fsw.prototype.connection_gone = function (why) {
  ...
  if( why === 'acl prevents connection') {
    logger.info('not reattempting connection due to ACL configuration on freeswitch: update freeswitch conf and retry') ;
    return ;
  }
  ...
  this.retry_timer = setTimeout(function () {
      ...
      if (self.connect_timeout && self.retry_totaltime >= self.connect_timeout) {
          ...
          logger.info("Fsw#connection_gone:: Couldn't get freeswitch connection after " + self.retry_totaltime + "ms.");
          return;
      }
      self.connect() ;
...
Fsw.prototype._onHeartbeat = function(evt) {
  ...
  logger.info('%s: sessions (max/current/avail): %d/%d/%d, cpu idle: %d', this.id, this.maxSessions, 
