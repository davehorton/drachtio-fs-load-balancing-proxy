...
Cluster.prototype._onOffline = function(fsw) {
  console.error('freeswitch %s went offline', Fsw.makeId(fsw)) ;
  this.emit('offline', fsw.toJSON() ) ;
} ;
Cluster.prototype._onOnline = function(fsw) {
  console.error('freeswitch %s went online', Fsw.makeId(fsw)) ;
  this.emit('online', fsw.toJSON() ) ;
} ;
Cluster.prototype._onReconnecting = function(fsw, obj) {
  console.error('freeswitch %s: reconnecting in %d ms (attempt #%d)', Fsw.makeId(fsw), obj.delay, obj.attempt) ;
} ;
