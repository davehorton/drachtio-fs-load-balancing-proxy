...
function Cluster() {
  ...
  fs.watchFile(configPath, function () {
    try {
      ...
      logger.info('Error re-reading config.js after modification; check to ensure there are no syntax errors: ', err) ;
    }
  }) ;
...
Cluster.prototype.addServer = function(targets, localAddress) {
  ...
  logger.info('added %d servers and removed %d servers', adds, remove.length) ;
} ;
